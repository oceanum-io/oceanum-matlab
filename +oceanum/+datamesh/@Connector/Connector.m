classdef Connector < handle
    ...
    methods
        ...
        function catalog = get_catalog(obj, search, timefilter, geofilter, limit)
          ...
          % limit - optional numeric limit on returned items
            ...
            if ~isConfigured(geofilter)
                if ischar(geofilter) || isstring(geofilter)
                    params(end+1) = matlab.net.QueryParameter('geom_intersects', char(geofilter)); %#ok<AGROW>
                elseif isstruct(geofilter) && isfield(geofilter,'wkt')
                    params(end+1) = matlab.net.QueryParameter('geom_intersects', char(geofilter.wkt)); %#ok<AGROW>
                else
         ...
         % --- helper (add inside same methods block or private methods block) ---
